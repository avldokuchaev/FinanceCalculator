# Калькулятор вклада с ежемесячной капитализацией
...
try:
    ...
    koefficient_dnei = procentnaya_stavka_modific / 12
    ...
    print(f"За {srok_vklada} месяца(ев) вы получите {round(zarabotan_summa_no_full, 2)}")
    print(f"Итоговая сумма через {srok_vklada} месяца(ев) составит {round(zarabotan_summa_full, 2)}")
